...
Suggested stack for live demo links:

1. **MongoDB Atlas** — free cluster; copy connection string to host env as `MONGO_URI`.
2. **API** — [Render](https://render.com) or [Railway](https://railway.app): root `server`, start `npm start`, set `MONGO_URI` and `PORT`.
3. **Frontend** — [Vercel](https://vercel.com): root `client`, build `npm run build`, output `dist`. Set env `VITE_API_URL=https://your-api.onrender.com`.

Update the [Live Demo Links](#live-demo-links) table once deployed.

---

## Git & Commits

Use **meaningful, atomic commits** — for example:

```
feat(server): add Task model and REST routes
feat(client): task list with filter and search
feat(client): add task form and edit flow
fix(api): return 400 for invalid MongoDB IDs
docs: complete README with API documentation
style(client): responsive layout and loading states
```

Do **not** commit `server/.env` (contains secrets). Use `.env.example` instead.
...
|------|--------|----------|
| Code quality | 25% | Separated routes, hooks, components; clear naming |
| Functionality | 25% | All core requirements implemented end-to-end |
| Full stack integration | 20% | REST API, validation, error handling on both sides |
| UI / UX | 15% | Loading skeletons, error alerts, mobile-responsive CSS |
| Documentation & Git | 10% | This README, API docs, setup steps, `.env.example` |
| Bonus / polish | 5% | Search, MongoDB persistence, a11y labels, dark mode |
